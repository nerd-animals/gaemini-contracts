...
(체결·호가·티커·펀딩·미결제약정) 및 저빈도 거시 관측치의 디스크 포맷 단일
진실 공급원.
...
용어
    ...
    exchange : 거래소 식별자. 소문자. 예) "upbit", "binance". 거래소가 아닌
               데이터 소스(거시 시계열 제공처)는 그 소스 id 가 온다 — 예)
               "fred", "ustreasury", "nyfed", "bok". "자리마다 내용물 그대로"
               불변식은 유지(경로 두 번째 자리 = 이 컬럼 값)이며, 의미만
               거래소 → 데이터 소스로 일반화한 것이다.
    symbol   : 거래소 원형 심볼. 예) "KRW-BTC"(upbit), "BTCUSDT"(binance).
               거시는 소스 시리즈 ID — 예) "DGS10"(fred).
...
"""거시 기반 시계열 관측치. 생산자 collector(FRED/US Treasury/NY Fed/BOK).

자리 매핑 (거래소 → 데이터 소스 일반화)::

    exchange           = 데이터 소스 id      예) "fred"
    symbol             = 소스 시리즈 ID      예) "DGS10"
    exchange_timestamp = **값이 알려진 시각** (KST naive) = 파티션·HWM 축
    observation_date   = 그 값이 가리키는 시점 (date-only, 소스 원본)
    value              = 관측치

`exchange_timestamp` 가 *관측 시점이 아니라 발표/조회 시점* 인 것이 핵심:
같은 ``observation_date`` 가 사후 개정되면 새 ``exchange_timestamp`` 로 다른
row 가 append 된다 → append-only 레일이 point-in-time(vintage) 이력을 그대로
보존한다. 개정 없는 시리즈는 둘이 사실상 같은 날을 가리킨다.
"""
